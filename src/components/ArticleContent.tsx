...
export default function ArticleContent({ article, initialLang }: ArticleContentProps) {
  ...
  const currentContent = initialLang === 'zh' ? zhContent || enContent : enContent || zhContent
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasBoth && (
          <div className="mb-6 text-sm text-gray-400">
            <a href={`?lang=${initialLang === 'zh' ? 'en' : 'zh'}`} className="hover:text-amber-500 transition-colors">
              {initialLang === 'zh' ? 'Read in English →' : '阅读中文版 →'}
            </a>
          </div>
        )}

        {/* Article body */}
        <div className={`prose max-w-none ${initialLang === 'zh' ? 'prose-zh' : ''}`}>
          <ReactMarkdown
